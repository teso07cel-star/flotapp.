      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-6">
                        <div className="flex items-center justify-between">
                          <h2 className="text-xl font-bold uppercase tracking-tight">Flota de Vehculos</h2>
                                  ...
                                  <tr>
                                    <td colSpan="3" className="p-10 text-center text-gray-500 font-medium">
                                      No hay vehculos registrados.
                                          ...
                                          const cubiertasCritico = kmParaCubiertas !== null && kmParaCubiertas <= 500;
  ...
  {!seguroVencido && seguroProximo && <span className="text-[8px] font-bold text-amber-600 uppercase">Seguro</span>}
   {serviceCritico && <span className="text-[8px] font-bold text-red-600 uppercase">Service</span>}
    {!serviceCritico && serviceProximo && <span className="text-[8px] font-bold text-amber-600 uppercase">Service</span>}
     {cubiertasCritico && <span className="text-[8px] font-bold text-red-600 uppercase">Cubiertas</span>}
      {!cubiertasCritico && cubiertasProximo && <span className="text-[8px] font-bold text-amber-600 uppercase">Cubiertas</span>}
        </div>
        </td>
                               <td className="p-5 pr-8 text-right">
                                 ...
                                 <form action={deleteVehiculoAction} className="inline">
                                     ...
                                     <button type="submit" className="h-8 w-8 flex items-center justify-center text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors border border-transparent hover:border-red-100" title="Borrar Vehculo">
   ...
   {registros.length === 0 ? (
                   <p className="text-gray-500 text-center py-4 font-medium italic">An no hay registros.</p>
               ) : registros.map((r) => (
...
{!r.novedadResuelta && (
                          <form action={resolverNovedadAction} className="flex gap-2">
                            <input type="hidden" name="id" value={r.id} />
                          <input name="resolucion" placeholder="Resolucin..." className="text-[8px] px-2 py-1 bg-white dark:bg-gray-950 border border-amber-200 dark:border-amber-500/30 rounded-md outline-none" required />
